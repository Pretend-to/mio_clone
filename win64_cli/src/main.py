...
def clone_github_repo(url):
    ...
    try:
        ...
        if 'downloadLink' in data:
            ...
            try:
                #print("开始下载文件...")
                download_command = ["wget", data['downloadLink'], "-O", f"{path.split('/')[-1]}.zip"]
                if platform.system() == "Windows":
                    download_command = ["curl", "-o", f"{path.split('/')[-1]}.zip", data['downloadLink']]

                subprocess.run(download_command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                #print("文件下载完成")
                ...
                #print("开始解压文件...")
                with zipfile.ZipFile(f"{path.split('/')[-1]}.zip", "r") as zip_ref:
